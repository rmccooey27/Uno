...
play_game<-function(num_players) {
  ...
  while (!done) {
    ...
    for (i in a:b) {
      ...
      if (status == "draw2") {
        print(paste0("Player ", i, " is drawing 2"))
        if ((length(deck[,1]) - 2) <= 1) {
          flip_deck(pile)
        }
        for (x in 1:2) {
        ...
        if ((length(deck[,1]) - 4) <= 1) {
          flip_deck(pile)
        }
        print(paste0("Player ", i, " is drawing 4"))
        for (x in 1:4) {
          player_decks[[i]][player_counts[i]+1,] <<- deck[x,]
          player_counts[i] <<- player_counts[i] + 1
        } 
        ...
        print(paste0("Player ", i, " has been skipped"))
        if (i == b) {
          i = a + 1
        } else if (a < b) {
          i = i + 1
      ...
      hand = player_decks[[i]]
      card = hand[move,]
      print(paste0("Player ", i, " has put down a: ", card[1], " ", card[2]))
      if (hand[move,1] == "wild") {
        ...
        print(paste0("Player ", i, " decided that the color is ", card[1]))
      } 
      ...
      if (player_counts[i] == 0) {
        print(player_counts)
        print(paste0("player ", i, " WINS!"))
        print("GAME OVER")
        done = TRUE
        return(done)
      } else if (player_counts[i] == 1) {
        print(paste0("Player ", i, " has UNO"))
      }
    }
  }
...
decide_move<-function(hand, hand_count, curr_num, curr_color, player_num) {
  ...
  while((!has_color) & (!found)) {
    ...
    print(paste0("Player ", player_num, " is picking a card"))
...
flip_deck<-function(pile) {
  print("flipping pile to make a new deck")
...
Uno<-function() {
  ...
  num_humans = readline(prompt="Enter the number of players: ")
  while (num_humans > 5 | num_humans <= 1) {
    num_humans = readline(prompt="You can have 2-5 players, try again: ")
  }
  num_humans = as.integer(num_humans)
  ...
  print("The cards have been dealt. Now it's time to play!")
  play_game(num_humans)
